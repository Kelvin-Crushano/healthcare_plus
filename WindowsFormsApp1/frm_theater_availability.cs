...
{
    public partial class frm_theater_availability : Form
    {
        ...
        String id;
        public frm_theater_availability()
        {
            InitializeComponent();
        }
        ...
        private void add()
        {
            ...
            cmd = new MySqlCommand("INSERT INTO `theater_schedule`(`pat_id`, `admission_date`, `type`, `theater_no`, `disarge_date`) VALUES (?,?,?,?,?)", con.open());
        ...
        private void load()
        {
            ...
            cmd = new MySqlCommand("SELECT * FROM `theater_schedule`", con.open());
            dare = cmd.ExecuteReader();
            while (dare.Read())
            {
                dataGridView1.Rows.Add(dare.GetString("id"), dare.GetString("pat_id"), dare.GetString("admission_date"), dare.GetString("type"), dare.GetString("theater_no"), dare.GetString("disarge_date"));
        ...
        private void dlt()
        {
            ...
            {
                cmd = new MySqlCommand("DELETE FROM `theater_schedule` WHERE `id` = ?", con.open());
        ...
        private void search()
        {
            ...
            cmd = new MySqlCommand("SELECT * FROM `theater_schedule` WHERE `theater_no`=?", con.open());
        ...
        private int find_availibility()
        {
            ...
            int status_ = 0;
            cmd = new MySqlCommand("SELECT * FROM `theater_schedule` WHERE `theater_no`=? AND `disarge_date`>=?", con.open());
        ...
        private void frm_theater_availability_Load(object sender, EventArgs e)
        {
